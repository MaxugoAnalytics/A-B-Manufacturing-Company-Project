...
st.markdown("""
    <h1 style="text-align: center; font-size: 2.5em; color: white; background-color: #4682B4; 
    padding: 20px; border-radius: 10px; margin-top: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
       A&B Manufacturing Company Sales Dashboard by Maxwell Adigwe
    </h1>
""", unsafe_allow_html=True)
...
def format_large_numbers(number):
    if number >= 1_000_000:
        return  f"₦{number / 1_000_000:.0f}M"  # Millions
    elif number >= 1_000:
        return f"₦{number / 1_000:.0f}K"  # Thousands
    else:
        return f"₦{number:.0f}"

# Calculate the totals
total_sales = round(filtered_df['total_sales'].sum(), 0)
total_profit = round(filtered_df['net_profit'].sum(), 0)
total_discounted_sales = round(filtered_df['discounted_sales'].sum(), 0)
total_units_sold = round(filtered_df['unit_sold'].sum(), 0)

formatted_total_sales = format_large_numbers(total_sales)
formatted_total_profit = format_large_numbers(total_profit)
formatted_total_discounted_sales = format_large_numbers(total_discounted_sales)
formatted_total_units_sold = total_units_sold

# Display KPIs
st.markdown(f"""
    <div class="kpi-container">
        <div class="kpi">
            <strong>Total Sales:</strong><br> {formatted_total_sales}
        </div>
        <div class="kpi">
            <strong>Total Profit:</strong><br> {formatted_total_profit}
        </div>
        <div class="kpi">
            <strong>Total Discounted Sales:</strong><br> {formatted_total_discounted_sales}
        </div>
        <div class="kpi">
            <strong>Total Units Sold:</strong><br> {formatted_total_units_sold}
        </div>
    </div>
""", unsafe_allow_html=True)
...
with col1:
    # Individual Filter for Profit Margin Plot without a filter title
    ...
    st.plotly_chart(fig2, height=100)
...
with col2:
    ...
    st.plotly_chart(fig3, height=100)
...
with col3:
    ...
    st.plotly_chart(fig4, height=100)
...
with col5:
    ...
    st.plotly_chart(fig5, height=100)
...
with col6:
    ...
    st.plotly_chart(fig1, height=100)
...
with col7:
    ...
    st.plotly_chart(fig7, height=100)
...
with col8:
    ...
    st.plotly_chart(fig8, height=100)
...
with col9:
    ...
    st.plotly_chart(fig9, height=100)
...
with col11:
    ...
    st.plotly_chart(fig11, height=100)
with col12:
    ...
    st.plotly_chart(fig12, height=100)
...
with col13:
    ...
    st.plotly_chart(fig13, height=100)
...
with col14:
    ...
    st.plotly_chart(fig14, height=100)
...
with col15:
    ...
    fig15 = px.bar(monthly_comparison, 
                   ...
                   barmode='group')
    st.plotly_chart(fig15, height=100) 
...
with col16:
    ...
    fig16 = px.bar(yearly_comparison, 
                   ...
                   barmode='group')
    st.plotly_chart(fig16, height=100)
